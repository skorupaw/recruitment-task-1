...
export function DictionaryCard({ emoji, title, word }: DictionaryCardProps) {
  ...
  return (
    <Card className="relative col-span-1 min-h-96 w-full overflow-hidden pb-0 shadow-lg lg:col-span-3">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 px-6 pb-2 pt-6">
        <div className="flex items-center gap-4">
          <div className="bg-muted flex h-12 w-12 items-center justify-center rounded-full">
            <span className="text-2xl" role="img" aria-label={`${title} emoji`}>
              {emoji}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" size="sm" className="text-muted-foreground h-8 px-2">
              <span className="text-sm font-normal">{word?.pronunciation}</span>
              <Volume2 className="ml-1 h-3.5 w-3.5" />
            </Button>
          </div>
          <Badge className="rounded-sm text-xs font-medium">{word?.partOfSpeech}</Badge>
        </div>
      ...
      <CardFooter className="bg-muted/30 px-6 py-4">
        <div className="text-muted-foreground text-xs">Source: Oxford English Dictionary*</div>
      </CardFooter>
    </Card>
  );
}
